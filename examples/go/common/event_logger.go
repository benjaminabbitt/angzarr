// Package common provides shared event logging utilities for projectors.
package common
...
func PrintEventDetails(eventType string, data []byte) {
	...
	case "DiscountApplied":
		...
		if err := goproto.Unmarshal(data, &event); err == nil {
			fmt.Printf("  %stype:%s     %s\n", Dim, Reset, event.DiscountType)
			fmt.Printf("  %svalue:%s    %d\n", Dim, Reset, event.Value)
			fmt.Printf("  %sdiscount:%s -$%.2f\n", Dim, Reset, float64(event.DiscountCents)/100)
			if event.CouponCode != "" {
				fmt.Printf("  %scoupon:%s   %s\n", Dim, Reset, event.CouponCode)
			}
		}

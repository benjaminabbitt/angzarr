...
public class TextRenderer {
  ...
  private String renderPlayerRegistered(PlayerRegistered event) {
    return String.format("Player registered: %s (%s)", event.getDisplayName(), event.getEmail());
  }
  ...
  private String renderTableCreated(TableCreated event) {
    return String.format(
        "Table created: %s (%d seats, blinds %d/%d)",
        event.getTableName(), event.getMaxPlayers(), event.getSmallBlind(), event.getBigBlind());
  }

  private String renderPlayerJoined(PlayerJoined event) {
    return String.format(
        "Player joined seat %d with %d chips", event.getSeatPosition(), event.getBuyInAmount());
  }

  private String renderPlayerLeft(PlayerLeft event) {
    return String.format(
        "Player left seat %d with %d chips", event.getSeatPosition(), event.getChipsCashedOut());
  }
  ...
  private String renderBlindPosted(BlindPosted event) {
    ...
    return String.format("%s posts %s blind: %d", player, event.getBlindType(), event.getAmount());
  }
  ...
  private String renderActionTaken(ActionTaken event) {
    ...
    if (event.getAmount() > 0) {
      return String.format(
          "%s %s %d (pot: %d)", player, action, event.getAmount(), event.getPotTotal());
    }
    return String.format("%s %s (pot: %d)", player, action, event.getPotTotal());
  }

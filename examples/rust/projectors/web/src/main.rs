//! Web projector gRPC server.
//!
//! Receives events and builds read models in PostgreSQL.
...
use projector_web::WebProjector;

/// gRPC service implementation wrapping the projector.
struct WebProjectorService {
    projector: WebProjector,
}

impl WebProjectorService {
    fn new(projector: WebProjector) -> Self {
        Self { projector }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let projector = WebProjector::new(pool);
    projector.init().await?;
    info!("Database schema initialized");

    let service = WebProjectorService::new(projector);

    // Create gRPC health service
    let (mut health_reporter, health_service) = tonic_health::server::health_reporter();
    health_reporter
        .set_serving::<ProjectorServer<WebProjectorService>>()
        .await;
    ...
    if transport_type == "uds" {
        ...
        let domain = env::var("DOMAIN").unwrap_or_else(|_| "web".to_string());
        ...
        info!(
            projector = projector_web::PROJECTOR_NAME,
        ...
        let port = env::var("PORT").unwrap_or_else(|_| "50153".to_string());

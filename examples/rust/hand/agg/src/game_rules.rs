...
fn evaluate_five(cards: &[&Card]) -> HandRank {
    ...
    if count_pattern == [2, 2, 1] {
        ...
        return HandRank::new(
            ...
            3_000_000 + high_pair * 1000 + low_pair * 50 + kicker,
    ...
    if count_pattern == [2, 1, 1, 1] {
        ...
        let kickers: Vec<i32> = counts[1..].iter().map(|(r, _)| *r).collect();
        let kicker_score =
            kickers[0] * 1000 + kickers.get(1).unwrap_or(&0) * 50 + kickers.get(2).unwrap_or(&0);
        return HandRank::new(
            HandRankType::Pair,
            2_000_000 + pair_rank * 10000 + kicker_score,

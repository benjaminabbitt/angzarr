...
private val logger = LoggerFactory.getLogger("FulfillmentSaga")
private const val SAGA_NAME = "fulfillment"
private const val SOURCE_DOMAIN = "order"

class FulfillmentSagaService : SagaGrpcKt.SagaCoroutineImplBase() {
    ...
    private fun processEvents(eventBook: EventBook): List<CommandBook> {
        ...
        for (page in eventBook.pagesList) {
            ...
            logger.info("triggering_fulfillment order_id={}", orderId)

            val createShipmentCmd = CreateShipment.newBuilder()
                .setOrderId(orderId)
                .build()
        ...
        if (commands.isNotEmpty()) {
            logger.info("processed_order_completed command_count={}", commands.size)
        }
...
fun main() {
    val port = System.getenv("PORT")?.toIntOrNull() ?: 50507

    val service = FulfillmentSagaService()

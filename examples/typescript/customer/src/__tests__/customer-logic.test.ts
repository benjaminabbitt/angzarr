...
import { CustomerLogic, EventBook } from '../customer-logic.js';
import { CustomerState, emptyCustomerState } from '../customer-state.js';
...
describe('CustomerLogic', () => {
  ...
  describe('handleCreateCustomer', () => {
    ...
    it('throws failed_precondition for existing customer', () => {
      ...
      expect(() => logic.handleCreateCustomer(state, 'New', 'new@test.com')).toThrow(
      ...
      try {
        ...
        expect((e as CommandValidationError).statusCode).toBe(
          StatusCode.FAILED_PRECONDITION
        );
        expect((e as CommandValidationError).message).toContain('already exists');
      }
    });
    ...
    it('throws invalid_argument for empty name', () => {
      ...
      expect(() => logic.handleCreateCustomer(state, '', 'email@test.com')).toThrow(
        CommandValidationError
      );
    ...
    it('throws invalid_argument for empty email', () => {
      ...
      expect(() => logic.handleCreateCustomer(state, 'Name', '')).toThrow(
        CommandValidationError
      );
  ...
  describe('handleAddLoyaltyPoints', () => {
    ...
    it('throws failed_precondition for non-existent customer', () => {
      ...
      expect(() => logic.handleAddLoyaltyPoints(state, 25, 'purchase')).toThrow(
        CommandValidationError
      );
    ...
    it('throws invalid_argument for zero points', () => {
      ...
      expect(() => logic.handleAddLoyaltyPoints(state, 0, 'purchase')).toThrow(
        CommandValidationError
      );
    ...
    it('throws invalid_argument for negative points', () => {
      ...
      expect(() => logic.handleAddLoyaltyPoints(state, -10, 'purchase')).toThrow(
        CommandValidationError
      );
    });
  });
  ...
  describe('handleRedeemLoyaltyPoints', () => {
    ...
    it('throws failed_precondition for non-existent customer', () => {
      ...
      expect(() => logic.handleRedeemLoyaltyPoints(state, 50, 'discount')).toThrow(
        CommandValidationError
      );
    ...
    it('throws failed_precondition for insufficient points', () => {
      ...
      try {
        ...
        expect((e as CommandValidationError).message).toContain('Insufficient');
      }
    });
    ...
    it('throws invalid_argument for zero points', () => {
      ...
      expect(() => logic.handleRedeemLoyaltyPoints(state, 0, 'discount')).toThrow(
        CommandValidationError
      );

...
function rebuildState(eventBook) {
    const state = {
        name: '',
        email: '',
        loyaltyPoints: 0,
    ...
    if (eventBook.snapshot?.state?.value) {
        try {
            const snapState = CustomerState.decode(eventBook.snapshot.state.value);
...
function nextSequence(priorEvents) {
    if (!priorEvents?.pages?.length)
        return 0;
...
function handleCreateCustomer(cmdBook, cmdData, state, seq) {
    if (state.name) {
        throw {
    ...
    const cmd = CreateCustomer.decode(cmdData);
    if (!cmd.name) {
        throw {
...
function handleAddLoyaltyPoints(cmdBook, cmdData, state, seq) {
    if (!state.name) {
        throw {
            code: grpc.status.FAILED_PRECONDITION,
    ...
    const cmd = AddLoyaltyPoints.decode(cmdData);
    if (cmd.points <= 0) {
        throw {
            code: grpc.status.INVALID_ARGUMENT,
    ...
    logger.info({ points: cmd.points, new_balance: newBalance, reason: cmd.reason }, 'adding loyalty points');
    const event = {
        points: cmd.points,
...
function handleRedeemLoyaltyPoints(cmdBook, cmdData, state, seq) {
    if (!state.name) {
        throw {
            code: grpc.status.FAILED_PRECONDITION,
            message: 'Customer does not exist',
    ...
    const cmd = RedeemLoyaltyPoints.decode(cmdData);
    if (cmd.points <= 0) {
        throw {
            code: grpc.status.INVALID_ARGUMENT,
            message: 'Points must be positive',
    ...
    const newBalance = state.loyaltyPoints - cmd.points;
    logger.info({ points: cmd.points, new_balance: newBalance, redemption_type: cmd.redemptionType }, 'redeeming loyalty points');
    const event = {
        points: cmd.points,
        newBalance: newBalance,

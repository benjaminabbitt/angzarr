/**
 * Fulfillment Saga - TypeScript Implementation
 *
 * Listens for OrderCompleted events and creates shipments.
 */
...
function processEvents(eventBook: any): any[] {
  ...
  for (const page of eventBook.pages) {
    ...
    logger.info({ orderId }, 'creating shipment for completed order');

    const createShipmentCmd = { orderId };
    const cmdAny = encodeToAny(CreateShipment, createShipmentCmd, 'examples.CreateShipment');
  ...
  Handle(
    ...
    try {
      ...
      if (commands.length > 0) {
        logger.info({ commandCount: commands.length }, 'processed order completion');
      }
      callback(null, {});
    } catch (err: any) {
...
async function main() {
  ...
  const port = process.env.PORT || '50407';

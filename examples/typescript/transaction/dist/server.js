...
function rebuildState(eventBook) {
    ...
    if (eventBook.snapshot?.state?.value) {
        try {
            const snapState = TransactionState.decode(eventBook.snapshot.state.value);
...
function handleCreateTransaction(cmdBook, cmdData, state, seq) {
    if (state.status) {
        throw { code: grpc.status.FAILED_PRECONDITION, message: 'Transaction already exists' };
    }
    const cmd = CreateTransaction.decode(cmdData);
    if (!cmd.customerId) {
...
function handleApplyDiscount(cmdBook, cmdData, state, seq) {
    if (state.status !== 'pending') {
        throw { code: grpc.status.FAILED_PRECONDITION, message: 'Transaction not in pending state' };
    }
    const cmd = ApplyDiscount.decode(cmdData);
    let discountCents = 0;
...
function handleCompleteTransaction(cmdBook, cmdData, state, seq) {
    if (state.status !== 'pending') {
        throw { code: grpc.status.FAILED_PRECONDITION, message: 'Transaction not in pending state' };
    }
    const cmd = CompleteTransaction.decode(cmdData);
...
function handleCancelTransaction(cmdBook, cmdData, state, seq) {
    if (state.status !== 'pending') {
        throw { code: grpc.status.FAILED_PRECONDITION, message: 'Transaction not in pending state' };
    }
    const cmd = CancelTransaction.decode(cmdData);

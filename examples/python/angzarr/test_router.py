...
class TestCommandRouterDispatch:
    ...
    def test_unknown_command_raises(self):
        router = CommandRouter(DOMAIN_TEST, dummy_rebuild).on(SUFFIX_COMMAND_A, handler_a)
    ...
    def test_handler_error_propagates(self):
        ...
        router = CommandRouter(DOMAIN_TEST, exists_rebuild).on(SUFFIX_CREATE, reject_handler)
    ...
    def test_suffix_matching_with_full_type_url(self):
        router = CommandRouter(DOMAIN_TEST, dummy_rebuild).on(SUFFIX_CREATE_ORDER, handler_a)
    ...
    def test_passes_args_to_handler(self):
        ...
        router = CommandRouter(DOMAIN_TEST, dummy_rebuild).on(SUFFIX_COMMAND_A, capturing_handler)
...
class TestCommandRouterMetadata:
    ...
    def test_descriptor(self):
        ...
        assert desc.inputs[0].event_types == [SUFFIX_CREATE_ORDER, SUFFIX_CANCEL_ORDER]
    ...
    def test_descriptor_type(self):
        ...
        assert isinstance(desc.inputs[0], SubscriptionDesc)
...
def saga_handler(event, root, correlation_id):
    return [
        angzarr.CommandBook(
            cover=angzarr.Cover(domain=DOMAIN_FULFILLMENT, root=root, correlation_id=correlation_id),
        ),
    ]


def multi_command_handler(event, root, correlation_id):
    return [
        angzarr.CommandBook(
            cover=angzarr.Cover(domain=DOMAIN_INVENTORY, root=root, correlation_id=correlation_id),
        ),
        angzarr.CommandBook(
            cover=angzarr.Cover(domain=DOMAIN_INVENTORY, root=root, correlation_id=correlation_id),
        ),
...
class TestEventRouterMetadata:
    ...
    def test_descriptor(self):
        ...
        assert len(desc.inputs) == 1
        assert desc.inputs[0].domain == DOMAIN_ORDER
        assert desc.inputs[0].event_types == [SUFFIX_ORDER_COMPLETED]

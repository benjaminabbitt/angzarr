defmodule Product.Server do
  ...
  defp dispatch_command(type_url, cmd_data, state) do
    cond do
      String.ends_with?(type_url, "CreateProduct") ->
        ...
        Logic.handle_create_product(state, cmd.sku, cmd.name, cmd.description, cmd.price_cents)

      String.ends_with?(type_url, "UpdateProduct") ->
        cmd = Examples.UpdateProduct.decode(cmd_data)
        log_info("updating_product", %{name: cmd.name})
        Logic.handle_update_product(state, cmd.name, cmd.description)

      String.ends_with?(type_url, "SetPrice") ->
        cmd = Examples.SetPrice.decode(cmd_data)
        log_info("setting_price", %{price_cents: cmd.price_cents})
        Logic.handle_set_price(state, cmd.price_cents)

      String.ends_with?(type_url, "Discontinue") ->
        cmd = Examples.Discontinue.decode(cmd_data)
        log_info("discontinuing_product", %{reason: cmd.reason})
        Logic.handle_discontinue(state, cmd.reason)

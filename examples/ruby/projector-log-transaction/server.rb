...
# Transaction Log Projector Service - Ruby Implementation
# Logs transaction domain events for audit/debugging.
...
DOMAIN = 'projector-log-transaction'

class TransactionLogProjectorService < Angzarr::ProjectorCoordinator::Service
...
def main
  port = ENV.fetch('PORT', '50076')

  server = GRPC::RpcServer.new
  server.add_http2_port("0.0.0.0:#{port}", :this_port_is_insecure)
  server.handle(TransactionLogProjectorService)

...
use crate::bus::AmqpEventBus;
use crate::config::SagaCompensationConfig;
use crate::interfaces::{BusError, EventBus, EventHandler};
...
use crate::services::saga_compensation::{build_revoke_command_book, CompensationContext};
...
pub struct SagaEventHandler {
    ...
    command_handler: Option<Arc<Mutex<BusinessCoordinatorClient<tonic::transport::Channel>>>>,
...
impl SagaEventHandler {
    ...
    pub fn with_command_handler(
        client: SagaCoordinatorClient<tonic::transport::Channel>,
        command_handler: BusinessCoordinatorClient<tonic::transport::Channel>,
        publisher: AmqpEventBus,
    ) -> Self {
        Self {
            client: Arc::new(Mutex::new(client)),
...
impl EventHandler for SagaEventHandler {
    fn handle(&self, book: Arc<EventBook>) -> BoxFuture<'static, Result<(), BusError>> {
        ...
        Box::pin(async move {
            ...
            // Call saga handle_sync to get resulting commands
            let mut client = client.lock().await;
            let response = client
                .handle_sync(book_to_send)
                .await
...
async fn handle_command_rejection(
    ...
    handler: &mut BusinessCoordinatorClient<tonic::transport::Channel>,
    publisher: &AmqpEventBus,
...
async fn emit_fallback_event(
    ...
    use crate::services::saga_compensation::build_compensation_failed_event_book;

...
| **[Projector](/components/projector)** | Events (multi-domain) | Side effects | Local only | Read models, external systems |
...
**Projectors never emit commands.** They only observe.
...
| **Domain subscription** | Single | Multiple | Multiple |
...
Upcasters transform old event versions to current versions when reading from the event store. This enables schema evolution without rewriting historical events.
...
- **Upcasters run on the client** — same gRPC server as aggregate logic

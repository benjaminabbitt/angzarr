...
```go file=examples/go/pmg-hand-flow/process.go start=docs:start:pm_state end=docs:end:pm_state
```
...
```python file=examples/python/pmg-hand-flow-oo/main.py start=docs:start:pm_state_oo end=docs:end:pm_state_oo
```

</TabItem>
<TabItem value="go" label="Go">

```go file=examples/go/pmg-hand-flow-oo/main.go start=docs:start:pm_state_oo end=docs:end:pm_state_oo
```

</TabItem>
<TabItem value="rust" label="Rust">

```rust file=examples/rust/pmg-hand-flow-oo/src/main.rs start=docs:start:pm_state_oo end=docs:end:pm_state_oo
```
...
```python file=examples/python/pmg-hand-flow-oo/main.py start=docs:start:pm_handler_oo end=docs:end:pm_handler_oo
```

</TabItem>
<TabItem value="go" label="Go">

```go file=examples/go/pmg-hand-flow-oo/main.go start=docs:start:pm_handler_oo end=docs:end:pm_handler_oo
```

</TabItem>
<TabItem value="rust" label="Rust">

```rust file=examples/rust/pmg-hand-flow-oo/src/main.rs start=docs:start:pm_handler_oo end=docs:end:pm_handler_oo
```
...
Facts are useful when the PM has authority the target aggregate must accept—phase transitions, timeouts, tournament rulings:
...
def handle_timeout(event: PlayerTimedOut, state: WorkflowState):
    return FactBook(
        domain="hand",
        external_id=f"timeout:{state.correlation_id}:{event.player_id}",
        event=PlayerForceFolded(player_id=event.player_id, reason="timeout"),
    )
```

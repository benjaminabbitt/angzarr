...
```java
CommandRouter router = new CommandRouter("player", StateBuilder::build)
    .on(RegisterPlayer.class, PlayerHandlers::handleRegister)
    .on(DepositFunds.class, PlayerHandlers::handleDeposit)
    .on(ReserveFunds.class, PlayerHandlers::handleReserve)
    .on(ReleaseFunds.class, PlayerHandlers::handleRelease);
```
...
```csharp
var router = new CommandRouter("player", StateBuilder.Build)
    .On<RegisterPlayer>(PlayerHandlers.HandleRegister)
    .On<DepositFunds>(PlayerHandlers.HandleDeposit)
    .On<ReserveFunds>(PlayerHandlers.HandleReserve)
    .On<ReleaseFunds>(PlayerHandlers.HandleRelease);
```
...
```cpp
auto router = CommandRouter("player", build_state)
    .on<RegisterPlayer>(handle_register)
    .on<DepositFunds>(handle_deposit)
    .on<ReserveFunds>(handle_reserve)
    .on<ReleaseFunds>(handle_release);

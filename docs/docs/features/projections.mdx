...
Projectors can subscribe to multiple domains when necessary. Use `input_domains` to declare domain subscriptions, `@handles` to route events to methods:

```python
class LeaderboardProjector(Projector):
    name = "projector-leaderboard"
    input_domains = ["player", "hand"]

...
Projectors can subscribe to multiple domains when necessary. Use `@subscribes` to declare domain subscriptions, `@handles` to route events to methods:

```python
@projector("leaderboard")
@subscribes("player", ["PlayerRegistered"])

...
```
Events (source of truth)     Projectors           Projections (query-optimized)
┌────────────────────┐      ┌──────────┐         ┌─────────────────────────┐
│ HandStarted        │      │          │         │ Redis: live table state │
│ PlayerActed        │ ──▶  │ Handlers │  ──▶    │ Postgres: hand history  │
│ PotAwarded         │      │          │         │ Elastic: player search  │
│ ...                │      └──────────┘         └─────────────────────────┘
```
...
| **Sync** | Read-after-write | Command waits for projector to complete |

For the table UI, async is fine—players tolerate milliseconds of lag. For "show me my updated balance after this deposit," sync ensures consistency.

```python
# Async: UI updates eventually
@projector("table-ui", sync=False)
class TableUIProjector: ...

# Sync: balance visible immediately
@projector("player-balance", sync=True)
class PlayerBalanceProjector: ...
```
...
Projections are disposable. Schema change? Bug fix? Rebuild:

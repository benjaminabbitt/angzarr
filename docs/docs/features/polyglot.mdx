...
public ActionRecorded Handle(PlayerAction cmd, HandState state)
{
    if (!state.IsPlayerTurn(cmd.PlayerId))
    {
        throw CommandRejectedError.PreconditionFailed("Not your turn");
    }
    return new ActionRecorded { ... };
}
```
...
```cpp
ActionTaken handle_action(const PlayerAction& cmd, const HandState& state) {
    if (!state.is_player_turn(cmd.player_id())) {
        throw angzarr::CommandRejectedError::precondition_failed("Not your turn");
    }
    return ActionTaken{...};
}
```

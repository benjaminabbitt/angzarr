...
| **Async** | Command returns immediately | High throughput, eventual consistency |

### Configuring Per-Projector

```python
# Balance updates must be visible immediately
@projector("player-balance", sync=True)
class BalanceProjector: ...

# Leaderboard can lag slightly
@projector("leaderboard", sync=False)
...
```python
# This specific command needs sync
result = send_command(
    DepositFunds(amount=1000),
    sync_projectors=["player-balance"],
)
# Balance is guaranteed visible after this returns
```

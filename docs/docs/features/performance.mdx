...
```
Without snapshots:
  Event 0 → Event 1 → ... → Event 99,999 → Current State
  (replay all 100,000 events)

With snapshots:
  Snapshot @ 99,000 → Event 99,001 → ... → Event 99,999 → Current State
  (replay only 999 events)
```
...
```
Table A events → Coordinator A → Table A aggregate
Table B events → Coordinator B → Table B aggregate
```

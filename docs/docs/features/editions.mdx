...
When you're done, delete the branch. Editions are for exploration, not migration—there's no merge operation that folds branch events into the main timeline.
...
```text title="illustrative - isolated experimentation"
...
A planned feature will add advice hooks for stripping personally identifiable information during event export, similar to the [upcasting](../sdks/upcasting) pattern. This will enable copying production events to isolated environments with automatic anonymization.

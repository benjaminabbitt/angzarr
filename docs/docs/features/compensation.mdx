...
```
1. Hand saga issues DeductFromPot → Table aggregate
2. Table rejects: "Player already folded"
3. Framework sends RejectionNotification to saga
4. Saga's source aggregate (Hand) receives notification
5. Hand emits compensation event: BetReverted
```
...
sequenceDiagram
    ...
    Table-->>Saga: Rejected (player folded)
    Saga->>Hand: RejectionNotification
    Hand->>Hand: BetReverted
```

The rejection flows back through the saga to the originating aggregate, which decides how to compensate.

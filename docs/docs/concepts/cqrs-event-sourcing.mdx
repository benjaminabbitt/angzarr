...
**Wrong (deltas):**
```python
# BAD: FundsReserved { amount: 500 }  # Delta: "reserve 500 more"
def apply(state, event):
    state.reserved += event.amount  # Double-apply = wrong state
```

**Correct (facts):**
```python
# GOOD: FundsReserved { amount: 500, new_reserved: 500, new_available: 500 }
def apply(state, event):

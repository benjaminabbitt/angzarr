...
- [EDA Visuals](https://eda-visuals.boyney.io/) by [@boyney123](https://twitter.com/boyney123) — Bite-sized diagrams explaining EDA concepts ([PDF](https://eda-visuals.boyney.io/visuals/eda-visuals.pdf))
  ...
  - [Bounded Context with Event Architectures](https://eda-visuals.boyney.io/visuals/bounded-context-with-event-architectures)
...
```mermaid
flowchart TB
    C1[1. Client sends Command]
    C2[2. Angzarr loads prior events]
    C3[3. Handler receives Command + Prior events]
    C4[4. Handler rebuilds current state]
    C5{5. Handler validates command}
    Err[Return error]
    C6[6. Handler returns new events]
    C7[7. Angzarr persists events]
    C8[8. Angzarr publishes events]
    Proj[Projectors]
    Saga[Sagas]

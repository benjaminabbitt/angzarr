...
:::note Code Examples
The code snippets throughout these feature pages are **illustrative**, not API reference. They show how concepts work, not exact function signatures. For working code, see the [SDK documentation](./sdks) and [example implementations](https://github.com/angzarr-io/angzarr/tree/main/examples).
:::
...
The payment cleared. The inventory reserved. Then the shipping provider rejected the address.

In a traditional system, you'd be unwinding database transactions, hoping nothing slipped through. Here, you emit a compensation event: `ShipmentFailed`. The saga that reserved inventory receives the rejection, and the source aggregate emits `InventoryReleased`. The audit trail shows exactly what happened and why.

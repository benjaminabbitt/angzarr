...
Command fails
    ...
    │                           └─ Still failing? ──→ DLQ
    │
    ├─ Sequence conflict? ──→ Check MergeStrategy
    │       │
    │       ├─ COMMUTATIVE ──→ Retry with fresh state
    │       ├─ STRICT ──→ Reject immediately
    │       ├─ AGGREGATE_HANDLES ──→ Let aggregate decide
    │       └─ MANUAL ──→ Send to DLQ
    │
    └─ Business rejection? ──→ Compensation flow (sagas)
```
...
message AngzarrDeadLetter {
  ...
  oneof payload {
    ...
    EventBook rejected_events = 9;      // Events that failed (saga/projector)
  }

  string rejection_reason = 3;          // Human-readable reason

  oneof rejection_details {
    SequenceMismatchDetails sequence_mismatch = 12;
    EventProcessingFailedDetails event_processing_failed = 13;
    PayloadRetrievalFailedDetails payload_retrieval_failed = 14;
  }

  Timestamp occurred_at = 7;
...
message RejectionNotification {
  CommandBook rejected_command = 1;     // Full command context
  string rejection_reason = 2;          // "insufficient_funds", "out_of_stock"
  string issuer_name = 3;               // Saga name
  string issuer_type = 4;               // "saga" | "process_manager"
  Cover source_aggregate = 5;           // Original triggering aggregate
  uint32 source_event_sequence = 6;     // Triggering event sequence
}
```

### Handling Rejections

The source aggregate can respond in several ways:

<Tabs groupId="language">
<TabItem value="python" label="Python (OO)" default>

```python
# In Player aggregate
class Player(Aggregate):
    @handles(Notification)
    def handle_notification(self, state: PlayerState, notification: Notification) -> Any:
        # Unpack the notification payload
        if notification.payload.Is(RejectionNotification.DESCRIPTOR):
            rejection = RejectionNotification()
            notification.payload.Unpack(rejection)
            return self.handle_rejection(state, rejection)
        return None  # Unknown notification type

    def handle_rejection(self, state: PlayerState, rejection: RejectionNotification) -> Any:
        # Check what was rejected
        if rejection.rejected_command.pages[0].command.Is(ReserveFunds.DESCRIPTOR):
            # Table join failed — release the reserved funds
            return FundsReleased(
                amount=self._extract_reserve_amount(rejection),
                reason=f"Join failed: {rejection.rejection_reason}",
            )

        # Unknown rejection — escalate
        return RevocationResponse(
            emit_system_revocation=True,
            send_to_dead_letter_queue=True,
            reason=f"Unhandled rejection: {rejection.rejection_reason}",
        )
```
...
```rust
// In Player aggregate handler
fn handle_notification(
    state: &PlayerState,
    notification: &Notification,
) -> Result<BusinessResponse, Status> {
    // Unpack notification payload
    let payload = notification.payload.as_ref()
        .ok_or_else(|| Status::invalid_argument("Missing payload"))?;

    if payload.type_url.ends_with("RejectionNotification") {
        let rejection: RejectionNotification = payload.unpack()?;
        return handle_rejection(state, &rejection);
    }
...
fn handle_rejection(
    ...
    let cmd_any = rejection.rejected_command.as_ref()
        .and_then(|c| c.pages.first())
        .and_then(|p| p.command.as_ref());

    if let Some(any) = cmd_any {
        if any.type_url.ends_with("ReserveFunds") {
            // Release reserved funds
            let event = FundsReleased {
                amount: extract_reserve_amount(rejection),
                reason: format!("Join failed: {}", rejection.rejection_reason),
            };
            return Ok(BusinessResponse::with_events(vec![event]));
        }
    }

    // Unknown rejection — escalate
    Ok(BusinessResponse::with_revocation(RevocationResponse {
        emit_system_revocation: true,
        send_to_dead_letter_queue: true,
        reason: format!("Unhandled rejection: {}", rejection.rejection_reason),
        ..Default::default()
    }))
}
```

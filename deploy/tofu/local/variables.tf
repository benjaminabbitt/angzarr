...
variable "messaging_type" {
  description = "Messaging backend: amqp or kafka"
  type        = string
  default     = "amqp"

  validation {
    condition     = contains(["amqp", "kafka"], var.messaging_type)
    error_message = "messaging_type must be either 'amqp' or 'kafka'"
  }
}

variable "enable_rabbitmq" {
  description = "Deploy RabbitMQ (defaults to true when messaging_type is amqp)"
  type        = bool
  default     = null
}
...
variable "enable_kafka" {
  description = "Deploy Kafka (defaults to true when messaging_type is kafka)"
  type        = bool
  default     = null
}

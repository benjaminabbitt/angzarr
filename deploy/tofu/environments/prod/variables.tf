...
# Passwords come from K8s secrets (synced via ESO from external secret stores)
...
variable "use_k8s_secrets" {
  description = "Read passwords from K8s secrets (synced via ESO). Set to false only for initial bootstrap."
  type        = bool
  default     = true
}
...
variable "database_managed" {
  description = "Use cloud-managed database (RDS, Cloud SQL, etc.)"
  type        = bool
  default     = true
}

# Fallback passwords (only used if use_k8s_secrets = false)
...
variable "messaging_managed" {
  description = "Use cloud-managed message broker (MSK, CloudAMQP, etc.)"
  type        = bool
  default     = true
}

# Fallback password (only used if use_k8s_secrets = false)
